...
- (NSMutableArray *)countAllMed{
  ...
  [queue inTransaction:^(FMDatabase *db, BOOL *rollback) {
    if ([db open]) {
      FMResultSet *medSet = [db executeQuery:@"SELECT * FROM Medicine"];
      while ([medSet next]) {
...
- (void)writeDetailByID:(NSString *)_ID {
  __block CHCSVWriter *csvWriter = [ExportTable sharedWriter];
  FMDatabaseQueue *queue = [dataBaseManager queue];
  NSArray *medArray = [self countAllMed];
  [queue inTransaction:^(FMDatabase *db, BOOL *rollback) {
    if ([db open]) {
      FMResultSet *resultSet2 = [db executeQuery:@"SELECT * FROM Detail WHERE Number = ?",_ID];
