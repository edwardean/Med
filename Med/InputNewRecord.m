...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (indexPath.row == self.selectIndex.row &&indexPath.section == self.selectIndex.section&& self.selectIndex != nil) {
    ...
    [self.field addTarget:self action:@selector(textfiledDoneEditing:)
         forControlEvents:UIControlEventEditingDidEndOnExit];
    self.field.delegate = self;
    
    UILabel *_danweiLabel = (UILabel *)[markCell.contentView
                                        viewWithTag:markDanweiLabel];
    ...
    [_okbtn setBackgroundImage:[UIImage imageNamed:Imgstr]
                      forState:UIControlStateNormal];
    if (![self.indexArray containsObject:indexPath]) {
      [_okbtn setBackgroundImage:ImageNamed(@"checkBtn")
                        forState:UIControlStateHighlighted];
    }
    [_okbtn addTarget:self action:@selector(okBtnClicked:)
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (!self.selectIndex) {
    ...
    if (!selectTheSameRow) {//两次点的行不同
      ...
      [tableView reloadRowsAtIndexPaths:[NSArray arrayWithObject:tempIndexPath]
                       withRowAnimation:UITableViewRowAnimationAutomatic];//合上上一次的行
      self.selectIndex = indexPath;
      [tableView reloadRowsAtIndexPaths:[NSArray arrayWithObject:self.selectIndex]
                       withRowAnimation:UITableViewRowAnimationAutomatic];//展开新点击的行
    } else {
      self.selectIndex = nil;
      [tableView reloadRowsAtIndexPaths:[NSArray arrayWithObject:indexPath]
                       withRowAnimation:UITableViewRowAnimationAutomatic];//直接合上点击的同一行
    }
  }
